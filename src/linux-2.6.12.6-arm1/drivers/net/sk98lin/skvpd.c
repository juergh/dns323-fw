...
static const char SysKonnectFileId[] =
	"@(#) $Id: skvpd.c,v 1.1.1.1 2009/08/19 08:02:20 jack Exp $ (C) Marvell.";
...
int		event)	/* event to wait for (VPD_READ / VPD_write) completion*/
{
	...
	start_time = 0; /* ronen */
	do {
		/* ronen - prevent stuck , since GetTime returns up to the Kernel HZ*2 at max */
		udelay(100);
		start_time++;
		if (start_time > 10000) /*SkOsGetTime(pAC) - start_time > SK_TICKS_PER_SEC) ronen */ {
