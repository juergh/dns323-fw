...
void GetSerialNumber(char *SerialNumber , char index); //jack20061109+

#define MV_ANY_PM {0, 0, 0, 0, 0}
#define MV_MARVELL_PM_A0 {0x11AB, 0x1116, 0, 0, 0}
#define MV_MARVELL_PM {0x11AB, 0x1116, 0xFF, 0, 0}
extern short PowerManagementSupport[2];  //jack20060426+

//jack20061109+
static char SerialNumber1[32];
static char SerialNumber2[32];
unsigned long SATAChannel = 0;
//jack20061109 end
                                 ...
                                 MV_U8 channelIndex )
{
    ...
    {
		SATAChannel |= 1;
		memset(SerialNumber1 , 0 , 32);
		for ( i=0;i<10;i++)
		{
			SerialNumber1[i*2]= (char)iden[19-i]&0xff;
			SerialNumber1[i*2+1]= (char)((iden[19-i]&0xff00)>>8);
		}
//		printk("Serial Number1 = ");
//		for ( i=0;i<20;i++)
//			printk("%c",SerialNumber1[19-i]);	
//		printk("\n");
		
//jack20061124+
		#ifdef SOFTWARE_CONTROL_LED
		vaule32 = mvGPIO_Get(0x10100);
		vaule32 &= 0xbfff;
		mvGPIO_Set(0x10100,vaule32); 
		#endif
//jack20061124 end
	}
	...
	{
		SATAChannel |= 2;
		memset(SerialNumber2 , 0 , 32);
		for ( i=0;i<10;i++)
		{
			SerialNumber2[i*2]= (char)iden[19-i]&0xff;
			SerialNumber2[i*2+1]= (char)((iden[19-i]&0xff00)>>8);
		}
//		printk("Serial Number2 = ");
//		for ( i=0;i<20;i++)
//			printk("%c",SerialNumber2[19-i]);
//		printk("\n");
//jack20061124+
		#ifdef SOFTWARE_CONTROL_LED
		vaule32 = mvGPIO_Get(0x10100);
...
//	printk("Serial Number = %x %x %x %x %x %x %x %x %x %x\n",iden[10],iden[11],iden[12],iden[13],
//	iden[14],iden[15],iden[16],iden[17],iden[18],iden[19]);
//jack20061109 end
    ...
    if ((iden[IDEN_SUPPORTED_COMMANDS1] & MV_BIT3))
    {
        ...
        if (iden[IDEN_ENABLED_COMMANDS1] & MV_BIT3)
        {
			...
			//printk("POWER MANAGMENT supported and enabled\n");
			PowerManagementSupport[channelIndex] = 1;
        }
        else
        {
            mvLogMsg(MV_IAL_COMMON_LOG_ID, MV_DEBUG, "%25s - %20s\n",
                     "POWER MANAGMENT", "supported and disabled");
			//printk("POWER MANAGMENT supported and disabled\n");     
			PowerManagementSupport[channelIndex] = 0;                  
        }
    ...
    else
    {
		...
		//printk("POWER MANAGMENT Not supported\n");
		PowerManagementSupport[channelIndex] = -1;
    }
...
#endif
    if (mvParseIdentifyResult(identifyBuffer, pIdentifyInfo , channelIndex ) == MV_FALSE)
    {
...
//jack20061109+
void GetSerialNumber(char *SerialNumber , char index)
{
	int i;
	...
	switch(index)
	{
		case 0:
			for ( i=0;i<20;i++)
				SerialNumber[i] = SerialNumber1[i];
			break;
		
		case 1:
			for ( i=0;i<20;i++)
				SerialNumber[i] = SerialNumber2[i];
			break;
...
void GetSATAChannelIndex( unsigned long *index )
{
	*index = SATAChannel;
}
