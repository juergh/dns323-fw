...
#ifndef OCF_RET_TASKLET
static struct	completion cryptoretproc_exited;
static DECLARE_WAIT_QUEUE_HEAD(cryptoretproc_wait);
static	int crypto_ret_proc(void *arg);
#else
static struct tasklet_struct ocf_ret_tasklet;
static void crypto_ret_tasklet(unsigned long dummy);
#endif
...
crypto_done(struct cryptop *crp)
{
	...
	if ((crp->crp_flags & CRYPTO_F_CBIMM) ||
		...
		if (wasempty) {
#ifdef OCF_RET_TASKLET
			tasklet_init(&ocf_ret_tasklet, crypto_ret_tasklet, (unsigned long)NULL);
			tasklet_schedule(&ocf_ret_tasklet);
#else
...
crypto_kdone(struct cryptkop *krp)
{
	...
	if ((krp->krp_flags & CRYPTO_KF_CBIMM)) {
		...
		if (wasempty) {
#ifdef OCF_RET_TASKLET
			tasklet_init(&ocf_ret_tasklet, crypto_ret_tasklet, (unsigned int) NULL);
			tasklet_schedule(&ocf_ret_tasklet);
#else
			wake_up_interruptible(&cryptoretproc_wait);	/*shared wait channel */
#endif
...
#else /* OCF_RET_TASKLET */
static void
crypto_ret_tasklet(unsigned long dummy)
{
	struct cryptop *crpt;
	struct cryptkop *krpt;
	unsigned long  r_flags;
